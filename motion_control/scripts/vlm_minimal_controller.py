...
--------------------
  motion_control/logs/vlm_minimal_<ts>.{csv,jsonl} — one row per VLM call
...
class VlmMinimalController:
    def __init__(self):
        rospy.init_node("vlm_minimal_controller", anonymous=False)
        ...
        # Lateral safety gate — VLM LEFT/RIGHT are advisory only.
        ...
        if self.goal_description and _BASE_SEQ_PROMPT is None:
            rospy.logerr(
                "[vlm_minimal] goal_description set but social_nav_eval_prompts "
        ...
        # --- Debug topic ---
        self.fsm_debug_topic = rospy.get_param("~fsm_debug_topic", "/vlm_minimal/fsm_debug")
        ...
        # --- cv_bridge + image sub ---
        if CvBridge is None:
            raise RuntimeError("cv_bridge unavailable; required for vlm_minimal_controller")
        ...
        self.vlm_jsonl_path = os.path.join(self.logs_dir, f"vlm_minimal_{ts}.jsonl")
        self.vlm_csv_path = os.path.join(self.logs_dir, f"vlm_minimal_{ts}.csv")
        ...
        print(f"[vlm_minimal] VLM decision log -> {self.vlm_csv_path}")
    ...
    def _build_social_nav_prompt(self):
        """Conservative social-nav prompt. VLM is a high-level decider
        only: ``recommended_action`` ∈ {STOP, FORWARD, REVIEW}. LEFT /
        RIGHT survive only as advisory hints in
        ``recommended_avoidance_side`` — the controller decides whether
        to execute them based on its own safety gate."""
        base = _BASE_SEQ_PROMPT or ""
        block = (
            ...
            "Prefer STOP over any suggestion of lateral motion "
            "whenever:\n"
            ...
            "latest frame after being absent earlier "
            "('entering_late').\n"
            "- REVIEW — REQUIRED in ALL of the following:\n"
            ...
            "    (c) a crossing person's direction is ambiguous,\n"
            ...
            "hypothetically prefer one side given depth information "
            "you do not actually have. Valid: left, right, none.\n"
            "- This field is a HINT. It does NOT make the robot "
            "strafe. Set it to `none` when uncertain.\n"
            "\nOUTPUT — extra JSON fields required:\n"
            "  \"person_bbox_height_frac\": float [0,1].\n"
            "  \"person_position\": center/left/right/none.\n"
            "  \"lateral_safety_verified\": false (always).\n"
            "  \"reasoning\": MANDATORY 1-3 sentences citing person "
            "apparent-size, position, motion, distance (with fisheye "
            "acknowledged). Never empty.\n"
            "\nFinal reminder: `recommended_action` MUST be one of "
            "{STOP, FORWARD, REVIEW}. If tempted to write LEFT or "
            "RIGHT, put the side in `recommended_avoidance_side` and "
            "set `recommended_action` to STOP or REVIEW.\n"
        )
        return base + block
    ...
    def fsm_enter(self, new_state):
        if self.fsm_state != new_state:
            rospy.loginfo("[vlm_minimal] %s -> %s (t=%.2f)", self.fsm_state, new_state,
    ...
    def step(self):
        ...
        if state == "DECIDE":
            ...
            if action is None:
                rospy.logwarn("[vlm_minimal] decide failed; holding")
            ...
            rospy.loginfo(
                "[vlm_minimal] decision=%s motion=%s cross_dir=%s avoid=%s goal_vis=%s goal_reached=%s reason=%s unc=%s",
            ...
            # LATERAL SAFETY GATE — LEFT/RIGHT are advisory hints only.
            ...
            if projected_action != action:
                rospy.logwarn(
                    "[vlm_minimal] VLM suggested %s (side=%s) — NOT executing: "
                    "%s",
            ...
            if action == "REVIEW":
                ...
                if (self.back_enabled
                    ...
                    rospy.logwarn(
                        "[vlm_minimal] stuck (%d REVIEW/STOP); emitting BACKWARD recovery",
            ...
            if vel is None:
                ...
                if (finished in ("LEFT", "RIGHT")
                    ...
                    if self._fsm_lateral_streak >= self.lateral_streak_max:
                        rospy.logwarn(
                            "[vlm_minimal] lateral streak cap %d reached -> HOLD_REVIEW",
        ...
        if state == "HOLD_REVIEW":
            ...
            if self.back_enabled:
                rospy.logwarn("[vlm_minimal] HOLD_REVIEW retries exhausted; BACKWARD recovery")
                self.fsm_review_count = 0
                self.begin_primitive("BACKWARD")
                self.fsm_enter("EXECUTE")
                return (-self.backward_speed, 0.0, 0.0), "BACKWARD", "VLM", \
                    "stuck-after-review BACKWARD recovery"
            rospy.logwarn("[vlm_minimal] REVIEW retries exhausted; auto-recovering to CRUISE")
            self.fsm_review_count = 0
            self.fsm_last_decision_time = 0.0
            self.fsm_enter("CRUISE")
            return (0.0, 0.0, 0.0), "STOP", "VLM_WAIT", "HOLD_REVIEW auto-recover -> CRUISE"

        rospy.logerr("[vlm_minimal] unknown state %s; forcing CRUISE", state)
        self.fsm_enter("CRUISE")
    ...
    def run(self):
        ...
        while not rospy.is_shutdown():
            ...
            if self._goal_reached:
                self.send_stop()
                print(
                    f"\033[1;32m[vlm_minimal] GOAL REACHED: "
